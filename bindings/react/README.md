...
const MyForm = () => {
  ...
  return (
    <form onSubmit={…}>
      {/* watching the values allows to rerun the children function when an item is added or removed */}
      <UseFieldArray form={form} path={'user.tags'} watch={['value']}>
        {({ path, value, add, remove }) => (
          <>
            <h6>Tags − <button onClick={() => add({})}>Add</button></h6>
            {value?.map((item, index) => (
              <div>
                <button onClick={() => remove(index)}>Remove</button>
                <UseField form={form} path={`${path}.${index}.label`}>
                  {({ path, ...state }) => (
                    <input name={path} {…} />
                  )}
                </UseField>

                <UseField form={form} path={`${path}.${index}.value`}>
                  {({ path, ...state }) => (
                    <input name={path} {…} />
                  )}
                </UseField>
